...
class NBLHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/health':
            return self.send_json({'ok': True, 'app': 'NBL Business Analyzer', 'version': 82})
...
def main():
    ...
    url = f'http://localhost:{actual_port}/index.html?v=82'
    if PORT_FILE:
        try:
            Path(PORT_FILE).write_text(url, encoding='utf-8')
        except Exception:
            pass
    print('NBL Business Analyzer v82 is running.')
    print(f'Open: {url}')
