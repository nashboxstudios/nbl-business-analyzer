...
class NBLHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/health':
            return self.send_json({'ok': True, 'app': 'NBL Business Analyzer', 'version': 83})
...
def main():
    ...
    url = f'http://localhost:{actual_port}/index.html?v=83'
    if PORT_FILE:
        try:
            Path(PORT_FILE).write_text(url, encoding='utf-8')
        except Exception:
            pass
    print('NBL Business Analyzer v83 is running.')
    print(f'Open: {url}')
